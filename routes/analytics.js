...
router.get('/', authenticate, authorize(['FleetManager', 'FinancialAnalyst', 'Dispatcher', 'SafetyOfficer']), async (req, res) => {
  try {
    ...
    const fuelEfficiency = totalFuelLiters > 0 ? (totalCompletedDistance / totalFuelLiters).toFixed(2) : '0.00';
    ...
    for (const vehicle of vehicles) {
      ...
      const roiPercent = vehicle.acquisitionCost > 0 ? ((netEarnings / vehicle.acquisitionCost) * 100).toFixed(2) : '0.00';
    ...
    const costliestVehicles = [...vehicleAnalyticsList]
      .sort((a, b) => b.totalCost - a.aTotalCost || b.totalCost - a.totalCost)
      .slice(0, 5);
